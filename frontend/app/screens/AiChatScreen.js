...
export default function AiChatScreen({ navigation }) {
  ...
  const handleSend = (text = inputText) => {
    ...
    setTimeout(() => {
      setIsTyping(false);
      const aiResponse = { id: (Date.now() + 1).toString(), text: `This is a simulated response to "${text}".`, sender: 'ai' };
  ...
  const renderMessage = ({ item }) => (
    ...
    >
      {item.sender === 'ai' && (
        <View style={styles.aiIconContainer}><Ionicons name="hardware-chip-outline" size={24} color="white" /></View>
      )}
      ...
      >
        ...
        {messages.length <= 1 && (
            <View style={[styles.suggestionContainer, {backgroundColor: theme.background}]}>
                {suggestedPrompts.map(prompt => (
                    <TouchableOpacity key={prompt} style={[styles.suggestionChip, {backgroundColor: theme.surface}]} onPress={() => handleSend(prompt)}>
        ...
        <View style={[styles.inputSection, {backgroundColor: theme.surface}]}>
            ...
            <View style={styles.inputButtonsRow}>
                <TouchableOpacity style={styles.iconButton}><Ionicons name="attach-outline" size={22} color={theme.textSecondary} /></TouchableOpacity>
                <TouchableOpacity style={styles.iconButton}><Ionicons name="calculator-outline" size={22} color={theme.textSecondary} /></TouchableOpacity>
                <TouchableOpacity style={styles.iconButton}><Ionicons name="options-outline" size={22} color={theme.textSecondary} /></TouchableOpacity>
            </View>
        </View>
      </KeyboardAvoidingView>
